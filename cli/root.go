// Package cli builds the hf command tree on top of the huggingface library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: huggingface.DefaultConfig()}

	root := &cobra.Command{
		Use:   "hf",
		Short: "Browse Hugging Face Hub models, datasets, and spaces",
		Long: `hf reads the Hugging Face Hub through its public REST API.
No API key is required for public content. It returns records as table,
JSON, JSONL, CSV, TSV, or URLs.

hf is an independent tool and is not affiliated with Hugging Face, Inc.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = huggingface.NewClient(a.cfg)
	return nil
}
