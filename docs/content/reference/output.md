...
describes what users get. Pick a format with `-o`, or let hf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hf <command> -o table   # aligned columns for reading
hf <command> -o jsonl   # one JSON object per line, for piping
hf <command> -o json    # a single JSON array
hf <command> -o csv     # spreadsheet friendly
hf <command> -o tsv     # tab-separated
hf <command> -o url     # just the URL column
hf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hf <command> --fields id,title,url
```
...
```bash
hf <command>            # a table, because this is a terminal
hf <command> | wc -l    # JSONL, because this is a pipe
```
